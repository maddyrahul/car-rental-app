...
export class MyRentalAgreementsComponent {
  ...
  ngOnInit(): void {
    const sharedResFromLocalStorage = localStorage.getItem('sharedRes');
    // Check if sharedResFromLocalStorage exists and is a valid number
    if (sharedResFromLocalStorage !== null) {
      this.sharedRes = +sharedResFromLocalStorage; // Parse as a number
      console.log(this.sharedRes)
    }
  ...
  loadRentalAgreements() {
    this.adminservice.getAllRentalcar().subscribe((data: RentalAgreement[]) => {
      ...
      this.rentalAgreements = data.filter(item => item.userId === this.sharedRes);
